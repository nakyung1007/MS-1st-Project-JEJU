...
const Gender = () => {
    ...
    useEffect(() => {
        if (userData.gender) {
            const newButtons = buttons.map(item => item.text === userData.gender ? {
                ...item,
                isClicked: true
            } : {...item, isClicked: false});
    ...
    useEffect(() => {
        let gender = null;
        buttons.map(item => item.isClicked === true ? gender = item.text : item);
                ...
                <ButtonContainer>
                    {
                        buttons.map((button, index) => (
                            <Button key={index} index={index} button={button} onClick={onClickGender}/>
                        ))
                    }
                </ButtonContainer>
            </div>
